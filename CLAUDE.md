...
L'œil du système : lit les 256 octets qu'emotion-emulator publie dans `/dev/shm/emotion-emulator`
et compose l'image projetée. Troisième pièce après crate et emotion-emulator ; **on n'y touche
...
- Le contrat (paquet, anneau) vit dans `emotion-emulator/src/Emotion.Signal/GpuPacket.cs` et
...
- Le rendu de référence est `emotion-emulator/outils/fenetre.py`. Ce qu'il fait, ce dépôt
  doit le faire pareil avant de faire mieux.
- Les règles du domaine sont dans le `CLAUDE.md` d'emotion-emulator (le son donne le
